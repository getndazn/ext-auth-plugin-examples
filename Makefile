...
GLOOE_VERSION ?= 1.4.11

# Set this variable to the name of your build plugin
PLUGIN_BUILD_NAME ?= GeoFencing.so
...
build-plugins-for-tests: $(EXAMPLES_DIR)/geofencing/GeoFencing.so

$(EXAMPLES_DIR)/geofencing/GeoFencing.so: $(SOURCES)
	go build -buildmode=plugin -o $(EXAMPLES_DIR)/geofencing/GeoFencing.so $(EXAMPLES_DIR)/geofencing/plugin.go
